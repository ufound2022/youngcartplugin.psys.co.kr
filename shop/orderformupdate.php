...
if (!empty($postEtc1)) {
    $sql = " select * from ".PSYSAPI_SESSION." where id={$postEtc1} ";
    $res = sql_fetch($sql);
    if ($res) {
...
// 결제 취소
function psysapi_cancel($tno) {
    global $psysapiApi;
    
    $return = false;

    $sql = " select * from ".PSYSAPI_PG_RESULT." where TID='{$tno}' ";
    $psysapi = sql_fetch($sql);
    if ($psysapi) {
        if (isset($psysapi['pay_type']) && !empty($psysapi['pay_type'])) {
            $psysapiApi = psysapi_get_api_account_info($default, $psysapi['pay_type']);
        }
        ...
        if ($ret['status'] === true) {
            @psysapi_payment_log("결제취소 성공", $ret['data'], 3);
        } else {
            @psysapi_payment_log("결제취소 실패", $ret['data'], 3);
        }
        ...
        if ($res) {
            @psysapi_payment_log("결제금액차이로 인한 결제취소결과 저장 성공", $sql, 3);
            $return = true;
        } else {
            @psysapi_payment_log("결제금액차이로 인한 결제취소결과 저장 실패", $sql, 3);
        }
    }
...
function psysapi_cancel_keyin($tno) {
    global $psysapiApiKeyin;
    ...
    $psysapi = sql_fetch($sql);
    if ($psysapi) {
        $ret = psysapi_cancel_payment($psysapiApiKeyin['api_id'], $psysapiApiKeyin['api_key'], $psysapi['TID']);

        if ($ret['status'] === true) {
            @psysapi_payment_log("수기결제 취소 성공", $ret['data'], 3);
        } else {
            @psysapi_payment_log("수기결제 취소 실패", $ret['data'], 3);
        }
...
{
    // 세션 생성
    $sql = " select ETC1 from ".PSYSAPI_PG_RESULT." where ORDERNO = {$od_id} ";
    $res = sql_fetch($sql);
    if ($res)
    {
        $sql = " select * from ".PSYSAPI_SESSION." where id={$res['ETC1']} ";
        $res = sql_fetch($sql);
        if ($res) {
            $arrSess = json_decode($res['json_sess'], true);
            foreach ($arrSess as $key => $val) {
    ...
    // 주문정보 설정
    $sql = " select * from ".PSYSAPI_SHOP_ORDER." where od_id = {$od_id} ";
    ...
    if ($order_data)
    {
        ...
        $od_send_cost = $_POST['od_send_cost']; // u: psysapi-plugin > 240315
        $_POST['od_send_cost2'] = $order_data['od_send_cost2'];
        $od_send_cost2 = $_POST['od_send_cost2']; // u: psysapi-plugin > 240315
...
else if ($od_settle_case == "계좌이체")
{
    switch($default['de_pg_service']) {
        ...
        default:
            ...
            // s: psysapi-plugin - 피시스API 처리
            if (array_key_exists($default['de_pg_service'], PSYSAPI_PG)) {
                require_once G5_PATH."/psysapi/pgresult.update.php";
                break;
            }
            // e: psysapi-plugin - 피시스API 처리
...
else if ($od_settle_case == "가상계좌" || $_POST['pay_method'] == "VACCT") // psysapi-plugin
{
    switch($default['de_pg_service']) {
        case 'lg':
            include G5_SHOP_PATH.'/lg/xpay_result.php';
...
else if ($od_settle_case == "신용카드" || $od_settle_case == "연구비카드결제") // psysapi-plugin
{
    switch($default['de_pg_service']) {
        case 'lg':
            include G5_SHOP_PATH.'/lg/xpay_result.php';
        ...
        default:
            ...
            // s: psysapi-plugin - 피시스API 처리
            if (array_key_exists($default['de_pg_service'], PSYSAPI_PG) || array_key_exists($default['de_pg_service'], PSYS_PG)) { // psysapi-plugin
                require_once G5_PATH."/psysapi/pgresult.update.php";
                @psysapi_payment_log("Debug", "신용카드결제후break시도", 3);
                break;
            }
            // e: psysapi-plugin - 피시스API 처리

            @psysapi_payment_log("Debug", "신용카드결제후break실패", 3);
    ...
    $pay_method         = $pay_method ? $pay_method : ''; // psysapi-plugin

    if($od_misu == 0)
        $od_status      = '입금';
}
// s: psysapi-plugin - 수기결제
else if ($od_settle_case == "수기결제") 
{
    ...
    ## s: psysapi-plugin 
    $pay_type           = $pay_type ? $pay_type : '';   // psysapi-plugin
    ## e: psysapi-plugin 
}
// e: psysapi-plugin
            ...
            default:
                ...
                if (array_key_exists($default['de_pg_service'], PSYSAPI_PG)) {
                    if ($od_settle_case == "수기결제") {
                        $psysapiCancelRes = psysapi_cancel_keyin($tno);
                    } else {
                        $psysapiCancelRes = psysapi_cancel($tno);
                    }
                    break;
                }
                // e: psysapi-plugin - 피시스API 처리
...
$sql = " insert {$g5['g5_shop_order_table']}
            set od_id             = '$od_id',
                ...
                // s : psysapi-plugin
                // od_casseqno      = '$od_casseqno' 추가
                // od_cash_info      = '$pay_method' 추가
                // e : psysapi-plugin
