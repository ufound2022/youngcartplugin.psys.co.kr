...
function check_pay_name_replace($payname, $od=array(), $is_client=0){
    ...
    } else if($payname == "연구비카드결제") { 
        
        if($payname == "연구비카드결제" && $od['od_cash_no'] == "1") { 
            $payname = "피시스 API - 연구비카드";
        } else
        if($payname == "연구비카드결제" && $od['od_cash_no'] == "3" && $od['od_cash_info'] == "CARD") { 
            $payname = "피시스 API - 일반카드";
        } else 
        if($payname == "연구비카드결제" && $od['od_cash_no'] == "3" && $od['od_cash_info'] == "NAVERPAY") { 
            $payname = "피시스 API - 네이버페이";
        } else 
        if($payname == "연구비카드결제" && $od['od_cash_no'] == "3" && $od['od_cash_info'] == "KAKAOPAY") { 
            $payname = "피시스 API - 카카오페이";
        } else                 
        if($payname == "연구비카드결제" && $od['od_cash_no'] == "3" && $od['od_cash_info'] == "BANK") { 
            $payname = "피시스 API - 계좌이체";
        } else 
        if($payname == "연구비카드결제" && $od['od_cash_no'] == "3" && $od['od_cash_info'] == "VACCT") { 
            $payname = "피시스 API - 가상계좌";
        } else                 
        if($payname == "연구비카드결제" && $od['od_cash_no'] == "4") { 
            $payname = "피시스 API - 간편결제(카드번호)";
        }
    }
