...
function form_submit(f)
{
    ...
    if($od['od_settle_case'] == "연구비카드결제" || $od['od_settle_case'] == "정기(구독)" || $od['od_settle_case'] == '계좌이체' || $od['od_settle_case'] == '신용카드' || $od['od_settle_case'] == '수기결제' || $od['od_settle_case'] == 'KAKAOPAY' || $od['od_settle_case'] == '간편결제' || ($od['od_pg'] == 'inicis' && is_inicis_order_pay($od['od_settle_case']) )) { 
    ...
    if(status == "취소" || status == "반품" || status == "품절") {
        ...
        /* s: psysapi-plugin */
        var psysapi_pg_chk = "<?=array_key_exists($default['de_pg_service'], PSYSAPI_PG)?>"; 
        /* s: psysapi-plugin */

        if(chk_cnt == chked_cnt || psysapi_pg_chk) { /* u: psysapi-plugin > 240315 */
