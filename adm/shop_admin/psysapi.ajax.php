...
if ($mode == "get") {
    ...
    $sql = " SELECT * FROM ".PSYSAPI_PG_RESULT." WHERE ORDERNO='{$orderNo}' AND RESULTCODE='0000' ORDER BY ACCEPTDATE DESC LIMIT 1 ";
    $res = sql_fetch($sql);
    if ($res) {
        $payAmount = $res['AMOUNT'];

        // 취소 내역 조회
        $sql2 = " SELECT COALESCE(SUM(cancel_amt), 0) AS cancel_amount FROM ".PSYSAPI_PG_CANCEL." WHERE orderno='{$orderNo}' ";
        $res2 = sql_fetch($sql2);
