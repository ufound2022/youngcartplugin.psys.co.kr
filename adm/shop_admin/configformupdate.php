...
$ssql = "SHOW COLUMNS FROM g5_shop_default LIKE 'de_psys_kw_id_research' ";
$sresult = sql_fetch($ssql);

if($sresult['Field'] == "de_psys_kw_id_research") { 
    #echo "필드 있음";
} else { 
    // 필드 생성

    // 피시스API PG사 연동 정보 컬럼 추가
    $sql = " ALTER TABLE `{$g5['g5_shop_default_table']}`
                ADD `de_psys_kw_id_research` VARCHAR(18) NOT NULL DEFAULT '' COMMENT '피시스 API > 키움 연구비카드 연동아이디' COLLATE 'utf8_general_ci' AFTER `de_pg_service`, 
                ADD `de_psys_kw_key_research` VARCHAR(50) NOT NULL DEFAULT '' COMMENT '피시스 API > 키움 연구비카드 연동키' COLLATE 'utf8_general_ci' AFTER `de_psys_kw_id_research`,
                ADD `de_psys_kw_id` VARCHAR(18) NOT NULL DEFAULT '' COMMENT '피시스 API > 키움 일반카드 연동아이디' COLLATE 'utf8_general_ci' AFTER `de_psys_kw_key_research` ,
                ADD `de_psys_kw_key` VARCHAR(50) NOT NULL DEFAULT '' COMMENT '피시스 API > 키움 일반카드 연동키' COLLATE 'utf8_general_ci' AFTER `de_psys_kw_id`, 
                ADD `de_psys_kw_id_keyin` VARCHAR(18) NOT NULL DEFAULT '' COMMENT '피시스 API > 키움 간편결제(카드번호) 연동아이디' COLLATE 'utf8_general_ci' AFTER `de_psys_kw_key` ,
                ADD `de_psys_kw_key_keyin` VARCHAR(50) NOT NULL DEFAULT '' COMMENT '피시스 API > 키움 간편결제(카드번호) 연동아이디' COLLATE 'utf8_general_ci' AFTER `de_psys_kw_id_keyin`, 
                ADD `de_psys_kw_id_web` VARCHAR(18) NOT NULL DEFAULT '' COMMENT '피시스 API > 키움 웹아이디' COLLATE 'utf8_general_ci' AFTER `de_psys_kw_key_keyin` ,
                ADD `de_psys_kw_key_web` VARCHAR(50) NOT NULL DEFAULT '' COMMENT '피시스 API > 키움 웹키' COLLATE 'utf8_general_ci' AFTER `de_psys_kw_id_web`, 
                ADD `de_psys_kw_card_type` VARCHAR(50) NOT NULL DEFAULT '' COMMENT '피시스 API > 사용할 결제타입' COLLATE 'utf8_general_ci' AFTER `de_psys_kw_key_web` ,

                ADD `de_psys_al_id_research` VARCHAR(18) NOT NULL DEFAULT '' COMMENT '피시스 API > 키움 연구비카드 연동아이디' COLLATE 'utf8_general_ci' AFTER `de_psys_kw_card_type`, 
                ADD `de_psys_al_key_research` VARCHAR(50) NOT NULL DEFAULT '' COMMENT '피시스 API > 키움 연구비카드 연동키' COLLATE 'utf8_general_ci' AFTER `de_psys_al_id_research`,
                ADD `de_psys_al_id` VARCHAR(18) NOT NULL DEFAULT '' COMMENT '피시스 API > 키움 일반카드 연동아이디' COLLATE 'utf8_general_ci' AFTER `de_psys_al_key_research` ,
                ADD `de_psys_al_key` VARCHAR(50) NOT NULL DEFAULT '' COMMENT '피시스 API > 키움 일반카드 연동키' COLLATE 'utf8_general_ci' AFTER `de_psys_al_id`, 
                ADD `de_psys_al_id_keyin` VARCHAR(18) NOT NULL DEFAULT '' COMMENT '피시스 API > 키움 간편결제(카드번호) 연동아이디' COLLATE 'utf8_general_ci' AFTER `de_psys_al_key` ,
                ADD `de_psys_al_key_keyin` VARCHAR(50) NOT NULL DEFAULT '' COMMENT '피시스 API > 키움 간편결제(카드번호) 연동아이디' COLLATE 'utf8_general_ci' AFTER `de_psys_al_id_keyin`, 
                ADD `de_psys_al_id_web` VARCHAR(18) NOT NULL DEFAULT '' COMMENT '피시스 API > 키움 웹아이디' COLLATE 'utf8_general_ci' AFTER `de_psys_al_key_keyin` ,
                ADD `de_psys_al_key_web` VARCHAR(50) NOT NULL DEFAULT '' COMMENT '피시스 API > 키움 웹키' COLLATE 'utf8_general_ci' AFTER `de_psys_al_id_web`, 
                ADD `de_psys_al_card_type` VARCHAR(50) NOT NULL DEFAULT '' COMMENT '피시스 API > 사용할 결제타입' COLLATE 'utf8_general_ci' AFTER `de_psys_al_key_web` ";
...
// s: psysapi-plugin
$check_sanitize_keys = array(
    'de_pg_service',                                // 피시스 api 결제대행사
    'de_psys_kw_id_research',                       // 1.비인증_연구비 연동아이디 // s: psysapi-plugin PSYS API
    'de_psys_kw_key_research',                      // 1.비인증_연구비 연동키
    'de_psys_kw_id',                                // 3.일반_인증 연동아이디
    'de_psys_kw_key',                               // 3.일반_인증 연동아이디
    'de_psys_kw_id_keyin',                          // 4.비인증_수기 연동아이디
    'de_psys_kw_key_keyin',                         // 4.비인증_수기 연동아이디
    'de_psys_kw_id_web',                            // 웹 연동아이디
    'de_psys_kw_key_web',                           // 웹 연동키
    'de_psys_kw_card_type',                            // PsyscardType 
    'de_psys_al_id_research',                       // 1.비인증_연구비 연동아이디 
    'de_psys_al_key_research',                      // 1.비인증_연구비 연동키
    'de_psys_al_id',                                // 3.일반_인증 연동아이디
    'de_psys_al_key',                               // 3.일반_인증 연동아이디
    'de_psys_al_id_keyin',                          // 4.비인증_수기 연동아이디
    'de_psys_al_key_keyin',                         // 4.비인증_수기 연동아이디
    'de_psys_al_id_web',                            // 웹 연동아이디
    'de_psys_al_key_web',                           // 웹 연동키
    'de_psys_al_card_type',                         // PsyscardType // e: psysapi-plugin PSYS API
...
$de_psys_kw_card_type = implode(',', $_POST['de_psys_kw_card_type']); 
$de_psys_al_card_type = implode(',', $_POST['de_psys_al_card_type']); 

foreach ($check_sanitize_keys as $column) {
    // s: psysapi-plugin PSYS API
    if (mb_strpos($de_pg_service, 'PSYS') !== false && isset($psys_de_pg_service) && !empty($psys_de_pg_service)) {
        $de_pg_service = $psys_de_pg_service;
    }     
    // e: psysapi-plugin PSYS API
    array_push($psysapiUpdateSet, "{$column}='{$$column}'");
}

$psysapiUpdateSetSql = implode(",", $psysapiUpdateSet);
...
// s: psysapi-plugin
if (count($psysapiUpdateSet) > 0) {
    $sql .= ",".$psysapiUpdateSetSql;
}
// e: psysapi-plugin
sql_query($sql);
