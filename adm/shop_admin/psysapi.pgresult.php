...
$sql = "SELECT (SELECT SUM(cancel_amt) FROM psysapi_pg_cancel WHERE orderno=R.ORDERNO) AS CANCEL_SUM_AMOUNT, C.cancel_tid, C.cancel_code, C.cancel_date, C.cancel_amt, R.*, V.PAYMETHOD, V.TID AS verify_tid, V.BUYERNAME, V.BUYEREMAIL, V.PRODUCTNAME, V.PRODUCTCODE, V.BUYERID, V.CARDCODE AS verify_cardcode FROM psysapi_pg_cancel AS C LEFT JOIN psysapi_pg_result AS R ON C.orderno=R.ORDERNO LEFT JOIN psysapi_pg_verify AS V ON C.orderno=V.ORDERNO WHERE C.cancel_code='0000' AND R.RESULTCODE='0000' AND V.RESULTCODE='0000' AND C.cancel_date BETWEEN '{$from}' AND '{$to}' ORDER BY C.cancel_date DESC, C.id DESC";
$res = sql_query($sql);
for($i=0; $row=sql_fetch_array($res); $i++) {
    ...
    $cancel[$i]['psysapiPg'] = mb_substr(PSYSAPI_PG[$pgUpper], 0, 2);
    $cancel[$i]['psysapiPg'] = $cancel[$i]['psysapiPg'] == '페이' ? '페누' : $cancel[$i]['psysapiPg'];

    // s: psysapi-plugin > 피시스 API
    if($row['PGNAME'] == "PSYS_AL" || $row['PGNAME'] == "PSYS_KW") { 
        $psysapi = psys_get_api_account_info_paytype($default, $row['pay_type']);
        $cancel[$i]['apiId'] = $psysapi['api_id'];
        $cancel[$i]['apiKey'] = $psysapi['api_key'];
    } else { 
...
$sql = "SELECT R.*, V.PAYMETHOD, V.TID AS verify_tid, V.BUYERNAME, V.BUYEREMAIL, V.PRODUCTNAME, V.PRODUCTCODE, V.BUYERID, V.CARDCODE AS verify_cardcode FROM psysapi_pg_result AS R LEFT JOIN psysapi_pg_verify AS V ON R.ORDERNO=V.ORDERNO WHERE R.RESULTCODE='0000' AND R.ACCEPTDATE BETWEEN '{$from}' AND '{$to}'  ORDER BY R.ACCEPTDATE DESC, R.id DESC";
$res = sql_query($sql);
for($i=0; $row=sql_fetch_array($res); $i++) {
    $success[$i]['psysapiPg'] = '';
    $success[$i]['apiId'] = '';
    ...
    $pgUpper = strtoupper($row['PGNAME']);
    $pgLower = strtolower($pgUpper);
    $success[$i]['psysapiPg'] = mb_substr(PSYSAPI_PG[$pgUpper], 0, 2);
    $success[$i]['psysapiPg'] = $success[$i]['psysapiPg'] == '페이' ? '페누' : $success[$i]['psysapiPg'];

    $psysapi = psysapi_get_api_account_info_by_pg($default, $pgLower, 3); // 신용카드
    ...
    switch ($row['PAYMETHOD']) {
        ...
        case 'CARD_SUGI':
            ...
            $psysapi = psysapi_get_api_account_info_by_pg($default, $pgLower, 1); // 수기결제
            break;
        default:
    ...
    if (!empty($success[$i]['pay_type'])) {
        $psysapi = psysapi_get_api_account_info_by_pg($default, $pgLower, $success[$i]['pay_type']); // 해외원화/달러인 경우
    }

    $success[$i]['apiId'] = $psysapi['api_id'];
    $success[$i]['apiKey'] = $psysapi['api_key'];
...
<link rel="stylesheet" href="<?php echo PSYSAPI_URL; ?>/datatable/jquery.dataTables.min.css">
<link rel="stylesheet" href="<?php echo PSYSAPI_URL ?>/modal/jquery.modal.min.css" />
...
<script src="<?php echo PSYSAPI_URL ?>/datatable/jquery.dataTables.min.js"></script>
<script src="<?php echo PSYSAPI_URL ?>/modal/jquery.modal.min.js"></script>
<script>
$(function(){
...
function receipt(tid) {
    ...
    window.open(
        "<?php echo PSYSAPI_RECEIPT_URL ?>?tid="+tid,
        "psysapi Receipt",
