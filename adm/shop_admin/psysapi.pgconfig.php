...
<form name="pgconfig" id="pgconfig" action="./psysapi.configformupdate.php" method="post">
<input type="hidden" name="token" value="">

<section id ="anc_scf_payment">
    <h2 class="h2_frm">피시스API설정</h2>
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            <th scope="row">
                ...
                <a href="<?php echo PSYSAPI_JOIN_URL ?>" target="_blank" class="kg_btn">피시스API 신청하기</a>
            </th>
            <td>
                ...
                $i = 0;
                foreach (PSYSAPI_PG as $pgCode => $pgName) {
                    $checked = "";
        ...
        <tr class="pg_info_fld <?php echo $pgCodeLower; ?>_info_fld" id="<?php echo $pgCodeLower; ?>_info_anchor">
            <th scope="row">
                <label for="de_<?php echo $pgCodeLower; ?>_psysapi_id"><?php echo $pgName; ?> 연동 아이디</label>
            </th>
            <td>
                <?php echo help("피시스API에서 발급 받으신 연동 아이디를 입력합니다."); ?>
                <input type="text" name="de_<?php echo $pgCodeLower; ?>_psysapi_id" value="<?php echo get_sanitize_input($default["de_{$pgCodeLower}_psysapi_id"]); ?>" id="de_<?php echo $pgCodeLower; ?>_psysapi_id" class="frm_input code_input" size="50" maxlength="50">
            </td>
        </tr>
        <tr class="pg_info_fld <?php echo $pgCodeLower; ?>_info_fld">
            <th scope="row"><label for="de_<?php echo $pgCodeLower; ?>_psysapi_key"><?php echo $pgName; ?> 연동 시크릿키</label></th>
            <td>
                <?php echo help("피시스API에서 발급 받으신 연동 시크릿키를 입력합니다."); ?>
                <input type="text" name="de_<?php echo $pgCodeLower; ?>_psysapi_key" value="<?php echo get_sanitize_input($default["de_{$pgCodeLower}_psysapi_key"]); ?>" id="de_<?php echo $pgCodeLower; ?>_psysapi_key" class="frm_input" size="67" maxlength="50">
            </td>
        </tr>
        <?php
            $i++;
...
var onSubmitConfirm = function(){
    $('#pgconfig').on("submit", function(e){
        ...
        var usePgId = $(`#de_${usePg}_psysapi_id`);
        var usePgKey = $(`#de_${usePg}_psysapi_key`);
