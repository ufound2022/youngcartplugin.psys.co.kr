...
// s: psysapi-plugin
$psysapi_order_status_info = array();
$cc = 0;
...
for ($i=0; $i<$cnt; $i++)
{
    ...
    // s: psysapi-plugin
    $psysapi_order_status_info[$cc]['ct_price'] = $ct['ct_price'] * $ct['ct_qty'];
    $cc++;
    // e: psysapi-plugin
...
if (in_array($_POST['ct_status'], $status_cancel)) {
    ...
    if( ($row['od_count1'] == $row['od_count2']) || ($row_c['od_count1'] > $row_c['od_count2']) ) {  // u: psysapi-plugin > 240314
        $cancel_change = true;
        ...
        if($pg_cancel == 1) {
            ...
            if($od['od_tno'] && ($od['od_settle_case'] == "연구비카드결제" || $od['od_settle_case'] == '계좌이체' || $od['od_settle_case'] == '신용카드' || $od['od_settle_case'] == '수기결제' || $od['od_settle_case'] == '간편결제' || $od['od_settle_case'] == 'KAKAOPAY') || ($od['od_pg'] == 'inicis' && is_inicis_order_pay($od['od_settle_case']) )) {
            ...
            // e: psysapi-plugin
                switch($od['od_pg']) {
                    ...
                    default:
                        ...
                        // s: psysapi-plugin add line, // s: psysapi-plugin > 피시스 API
                        if (array_key_exists($default['de_pg_service'], PSYSAPI_PG) || array_key_exists($default['de_pg_service'], PSYS_PG)) {

                            if( ($row['od_count1'] == $row['od_count2']) && ($row_c['od_count2'] == 0) ) { 
                                $psysapi_full_cancel = 1;
                                require_once G5_PATH."/adm/shop_admin/psysapi.cancel.php";
                            } else
                ...
                // u: psysapi-plugin > 240315, // psysapi-plugin
                // PG 취소요청 성공했으면
                if (array_key_exists($default['de_pg_service'], PSYSAPI_PG) || array_key_exists($default['de_pg_service'], PSYS_PG)) { 

                    if($pg_res_cd == '' && $psysapi_full_cancel == "1") {
