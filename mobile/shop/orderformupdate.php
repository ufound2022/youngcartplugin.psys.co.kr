...
// api account
$psysApi = psysapi_get_api_account_info($default, 3);

// keyin api account
$psysApiKeyin = psysapi_get_api_account_info($default, 1);
...
function psysapi_cancel($tno) {
    ...
    if ($psysapi) {
        if (isset($psysapi['pay_type']) && !empty($psysapi['pay_type'])) {
            $psysApi = psysapi_get_api_account_info($default, $psysapi['pay_type']);
        }

        $ret = psysapi_cancel_payment($psysApi['api_id'], $psysApi['api_key'], $psysapi['TID'], $psysapi['CARDCODE'], $psysapi['ACCOUNTNO'], $psysapi['RECEIVERNAME']);
...
function psysapi_cancel_keyin($tno) {
    ...
    $psysapi = sql_fetch($sql);
    if ($psysapi) {
        $ret = psysapi_cancel_payment($psysApiKeyin['api_id'], $psysApiKeyin['api_key'], $psysapi['TID']);
...
else if ($od_settle_case == "신용카드" || $od_settle_case == "연구비카드결제")
{
    switch($default['de_pg_service']) {
        case 'lg':
            include G5_SHOP_PATH.'/lg/xpay_result.php';
