...
<div id="sod_fin">
    ...
    <div id="sod_fin_view">
        ...
        if($od['od_settle_case'] == '연구비카드결제' || $od['od_settle_case'] == '정기(구독)' || $od['od_settle_case'] == '신용카드' || $od['od_settle_case'] == '수기결제' || $od['od_settle_case'] == 'KAKAOPAY' || is_inicis_order_pay($od['od_settle_case']) ) {
        // e: psysapi-plugin
            $app_no_subj = '승인번호';
            $app_no = $od['od_app_no'];
            $disp_bank = false;
        ...
        <section id="sod_fin_pay">
            ...
            <div  class="odf_tbl">
                ...
                // 계좌정보
                // s : psysapi-plugin
                if($disp_bank && ($od['od_cash_info'] != "1" && $od['od_cash_info'] != "3" && $od['od_cash_info'] != "4"))
                // e : psysapi-plugin
                {
                ?>
                <tr>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        // s: psysapi-plugin, // s: psysapi-plugin > 피시스 API
                        if($od['od_settle_case'] == '연구비카드결제' || $od['od_settle_case'] == '정기(구독)' || $od['od_settle_case'] == '신용카드' || $od['od_settle_case'] == '수기결제' || is_inicis_order_pay($od['od_settle_case']) || (shop_is_taxsave($od, true) && $misu_price == 0) )
                        {
                        // e: psysapi-plugin, // e: psysapi-plugin > 피시스 API
                            if($od['od_pg'] == 'lg') {
                                require_once G5_SHOP_PATH.'/settle_lg.inc.php';
                                $LGD_TID      = $od['od_tno'];
                                $LGD_MERTKEY  = $config['cf_lg_mert_key'];
                                ...
                                if (array_key_exists($default['de_pg_service'], PSYSAPI_PG)) {
                                    $card_receipt_script = "psysapiReceipt('".$od['od_tno']."');";
                                }
...
// s: psysapi-plugin - 피시스API 처리
function psysapiBase64Encode(str) {
    return btoa(encodeURIComponent(str).replace(/%([0-9A-F]{2})/g, function(match, p1) {
        return String.fromCharCode('0x' + p1);
    }));
}

function psysapiReceipt(tid) {
    var tid = psysapiBase64Encode(tid);
    window.open(
