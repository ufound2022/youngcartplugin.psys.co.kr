...
// 결제 결과 테이블 없으면 생성
if(!sql_query(" DESCRIBE ".PSYSAPI_PG_RESULT." ", false)) {
    $sql = " CREATE TABLE `".PSYSAPI_PG_RESULT."` (
            ...
            COMMENT='피시스API 플러그인 결제 응답 결과'
            COLLATE='utf8mb4_general_ci'
            ENGINE=InnoDB ";
    sql_query($sql, true);
}

//@sql_query(" ALTER TABLE `".PSYSAPI_PG_RESULT."` ADD COLUMN IF NOT EXISTS `pay_type` VARCHAR(2) NOT NULL COMMENT '결제타입. 1:수기결제,3:신용카드인증,5:해외달러결제,7:해외원화결제' COLLATE 'utf8_general_ci' AFTER `PGNAME` ", true);
//@sql_query(" ALTER TABLE `".PSYSAPI_PG_RESULT."` ADD COLUMN IF NOT EXISTS `pay_status` TINYINT NOT NULL DEFAULT 0 COMMENT '결제상태. 0:결제시도,1:결제성공,2:결제취소,3:결제실패,4:가상계좌' AFTER `pay_type` ", true);
...
AND TABLE_NAME='".PSYSAPI_PG_RESULT."'
AND information_schema.`COLUMNS`.COLUMN_NAME = 'pay_type';
";
$res = sql_fetch($sql);
if (!isset($res['COLUMN_NAME']))
{
    @sql_query(" ALTER TABLE `".PSYSAPI_PG_RESULT."` ADD COLUMN `pay_type` VARCHAR(2) NOT NULL COMMENT '결제타입. 1:수기결제,3:신용카드인증,5:해외달러결제,7:해외원화결제' COLLATE 'utf8mb4_general_ci' AFTER `PGNAME` ", true);
}
...
if (!isset($res['COLUMN_NAME']))
{
    @sql_query(" ALTER TABLE `".PSYSAPI_PG_RESULT."` ADD COLUMN `pay_status` TINYINT NOT NULL DEFAULT 0 COMMENT '결제상태. 0:결제시도,1:결제성공,2:결제취소,3:결제실패,4:가상계좌' AFTER `pay_type` ", true);
}
...
if (isset($res))
{
    ...
    {
        @sql_query(" ALTER TABLE `".PSYSAPI_PG_RESULT."` CHANGE COLUMN `TID` `TID` VARCHAR(100) NOT NULL COMMENT '거래고유번호. pg사 결제 거래고유번호 (전표출력 및 결제취소에 사용됩니다)' COLLATE 'utf8mb4_general_ci' AFTER `AMOUNT` ", true);
    }
}
// e: psysapi-plugin
...
// 결제 검증 테이블 없으면 생성
if(!sql_query(" DESCRIBE ".PSYSAPI_PG_VERIFY." ", false)) {
    $sql = " CREATE TABLE `".PSYSAPI_PG_VERIFY."` (
                `id` INT(11) NOT NULL AUTO_INCREMENT COMMENT '고유번호',
...
// 결제 취소 테이블 없으면 생성
if(!sql_query(" DESCRIBE ".PSYSAPI_PG_CANCEL." ", false)) {
    $sql = " CREATE TABLE `".PSYSAPI_PG_CANCEL."` (
                `id` INT(11) NOT NULL AUTO_INCREMENT COMMENT '고유번호',
                `orderno` VARCHAR(50) NOT NULL COMMENT '주문번호' COLLATE 'utf8mb4_general_ci',
...
if(!sql_query(" DESCRIBE ".PSYSAPI_SHOP_ORDER." ", false)) {
    $sql = " CREATE TABLE `".PSYSAPI_SHOP_ORDER."` (
