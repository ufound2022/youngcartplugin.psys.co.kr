...
function pay_psysapi(payType) {
    ...
    var psysapi_pay_url = "";
    if(payType == "수기결제") { 
        psysapi_pay_url = "psysapi.pgwin.php";
    }
    ...
    if (payType == "수기결제") {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo PSYSAPI_URL ?>/psysapi.pay.php",
            data : tryOrderParams,
            cache: false,
            ...
            success: function(result) {
                try {
                    ...
                    <?php if ($default['de_pg_service'] == 'PSYSAPI_PN') { ?>
        ...
        var pgWin1 = window.open(`<?php echo PSYSAPI_URL; ?>/`+psysapi_pay_url+`?pm=${payType}&pt=${pt}&it_id=<?=$row2['it_id']?>`, "pgWin1", popupPos);
        ...
        <?php
        $psysaApi = psysapi_get_api_account_info($default, 3);
        $psysaApi_won = psysapi_get_api_account_info($default, 7);
        ?>
        var payMethod = '';
        switch (payType) {
            case '간편결제':
        ...
        if (isTest == '0') {
            var url = "<?php echo PSYSAPI_PAY_URL; ?>"; // 실결제
        } else {
            var url = "<?php echo PSYSAPI_TESTPAY_URL; ?>"; // 테스트결제
        }
        ...
        var params = {
            API_ID: "<?php echo $psysaApi['api_id']; ?>",
            API_ID_WON: "<?php echo $psysaApi_won['api_id']; ?>",
                    ...
                    success: function(tryOrderRes) {
                        $.ajax({
                            ...
                            beforeSend : function(xhr) {
                                xhr.setRequestHeader("ApiKey", "<?php echo $psysaApi['api_key']; ?>");
                            },
                            success: function(result) {
                                try {
                                    const obj = JSON.parse(result);
